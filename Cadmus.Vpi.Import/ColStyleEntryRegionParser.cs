...
/// <summary>
/// VPI column style entry region parser.
/// </summary>
/// <seealso cref="EntryRegionParser" />
/// <seealso cref="IEntryRegionParser" />
[Tag("entry-region-parser.vpi.col-style")]
public sealed class ColStyleEntryRegionParser :
    EntryRegionParser, IEntryRegionParser
{
    /// <summary>
    /// Gets the tags of the regions that this parser can handle.
    /// </summary>
    public string[] RegionTags => ["col-artistic_style"];
        ...
        IReadOnlyList<EntryRegion> entryRegions, int entryRegionIndex)
    {
        ...
        {
            Logger?.LogError("col-artistic_style column without any item at region {Region}",
                region);
            throw new InvalidOperationException(
                "col-artistic_style column without any item at region " + region);
        }
        ...
        if (!string.IsNullOrEmpty(value))
        {
            ...
            if (thesaurus == null)
            {
                Logger?.LogError("col-artistic_style value {Value} not checked in " +
                    "thesaurus categories_wblk@en at region {Region}",
                    value, region);
            }
            else
            {
                ...
                {
                    Logger?.LogError("col-artistic_style value {Value} not found in " +
                        "thesaurus categories_wblk@en at region {Region}",
                        value, region);
                }
